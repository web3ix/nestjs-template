...
export function ArrayField<T>(
  getItemType: () => Constructor<T> | PropertyDecorator,
  ...
  return (target: object, propertyKey: string | symbol) => {
    const decorators: PropertyDecorator[] = [IsArray({ each: options.each })];

    // Handle the item type
    if (typeof getItemType === 'function') {
      const itemType = getItemType();
      if ('prototype' in itemType) {
        // It's a constructor (class)
        decorators.push(Type(() => itemType as Constructor<T>));
        // decorators.push(ValidateNested({ each: true }));
      } else {
        // It's a decorator (like StringField, NumberField, etc.)
        decorators.push(itemType as PropertyDecorator);
      }
    }

    if (options.nullable) {
      decorators.push(IsNullable({ each: true }));
    } else {
    ...
    if (options.swagger !== false) {
      ...
      decorators.push(
        ApiProperty({
          type: () =>
            typeof getItemType === 'function' && 'prototype' in getItemType()
              ? [getItemType()]
              : Array,
          required: !!required,
          isArray: true,
    ...
    if (typeof options.minItems === 'number') {
      decorators.push(MinLength(options.minItems));
    }

    if (typeof options.maxItems === 'number') {
      decorators.push(MaxLength(options.maxItems));
    }
